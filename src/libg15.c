...
int re_initLibG15() {
	...
	/*
	 * usb_find_busses and usb_find_devices both report the number of devices/busses
	 * added/removed since the last call. Since this is the first call we have to
	 * return values != 0 or else we didnt find anything
	 */
...
int writePixmapToLCD(unsigned char const *data) {
	...
	/*
	 * The pixmap conversion function will overwrite everything after G15_LCD_OFFSET,
	 * so we only need to blank the buffer up to this point. (Even though the keyboard
	 * only cares about bytes 0-23.)
	 */
	...
	lcd_buffer[0] = 0x03;
	/*
	 * In an attempt to reduce peak bus utilisation, we break the transfer into 32 byte
	 * chunks and sleep a bit in between. It shouldnt make much difference, but then
	 * again, the g15 shouldnt be flooding the bus enough to cause ENOSPC, yet apparently
	 * does on some machines...
	 * I'm not sure how successful this will be in combatting ENOSPC, but we'll give
	 * it try in the real-world.
	 */
